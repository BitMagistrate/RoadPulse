...
class RoutingEngine:
    ...
    def shortest(
        ...
        *,
        ...
        return self._reconstruct(origin_id, destination_id, prev, profile, locale=locale)
    ...
    def _reconstruct(
        ...
        *,
        ...
        for src_node_id, edge_idx in zip(path_nodes[:-1], edge_sequence, strict=True):
            ...
            road_name = edge.tags.get("name") or _class_label(edge.road_class, locale=locale)
            instruction = _build_instruction(
                road_name, edge.distance_m, edge.road_class, locale=locale
            )
...
_ROAD_CLASS_LABELS: dict[str, dict[str, str]] = {
    "en": {
        "motorway": "highway",
        "trunk": "boulevard",
        "primary": "avenue",
...
_INSTRUCTION_TEMPLATES: dict[str, str] = {
    "vi": "Đi tiếp trên {label} thêm {dist}",
    "en": "Continue on {label} for {dist}",
}


def _class_label(road_class: str, *, locale: str = "vi") -> str:
    table = _ROAD_CLASS_LABELS.get(locale) or _ROAD_CLASS_LABELS["en"]
    return table.get(road_class, table.get("unclassified", "road"))
...
def _build_instruction(
    ...
    *,
    ...
    """Human-readable turn-by-turn instruction localised to ``locale``."""
    label = road_name if road_name else _class_label(road_class, locale=locale)
    if distance_m >= 1_000:
        dist_str = f"{distance_m / 1_000:.1f} km"
    else:
        dist_str = f"{int(round(distance_m))} m"
    template = _INSTRUCTION_TEMPLATES.get(locale) or _INSTRUCTION_TEMPLATES["en"]
    return template.format(label=label, dist=dist_str)
