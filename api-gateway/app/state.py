...
class AppState:
    ...
    def __init__(self, seed_dir: Path | None = None) -> None:
        ...
        # --- Crowd-sourced flood reports (M3) ---------------------------------
        self.crowd_boost: dict[str, float] = {}
        self.flood_reports: deque[dict[str, Any]] = deque(maxlen=500)
        self.flood_report_last_seen: dict[str, datetime] = {}

        # --- Geocode places (M4) ----------------------------------------------
        self.places: list[dict[str, Any]] = self._load_places()

        # Seed the k-anon guard with a few synthetic violations so the admin
        # page has something to render in the demo (data_origin=synthetic).
        self._seed_kanon_violations()
    ...
    def _load_places(self) -> list[dict[str, Any]]:
        candidate = Path(__file__).resolve().parents[3] / "data" / "seed" / "places.json"
    ...
    def _seed_kanon_violations(self) -> None:
        # A handful of synthetic violations across different buckets so the
        # B2B admin page has something to render. The guard keeps the most
        # recent entries via its in-memory sink.
        now = datetime.now(UTC)
        seeds: list[tuple[str, int, timedelta]] = [
            ("hex_flood_d1_07/heatmap", 12, timedelta(minutes=2)),
            ("fleet_corridor_q3/eta_export", 31, timedelta(minutes=14)),
            ("policy_pti_d1_flood_2024/trigger_replay", 5, timedelta(minutes=33)),
            ("isochrone_export_d7/15min", 23, timedelta(minutes=58)),
            ("flood_overlay/public_csv", 41, timedelta(hours=2)),
        ]
        for bucket, k, ago in seeds:
            self.kanon_guard.check(bucket=bucket, observed_k=k, at=now - ago)
