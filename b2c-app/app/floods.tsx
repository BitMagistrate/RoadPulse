...
import { api, type FloodOverlayPoint } from "@/lib/api";
...
export default function FloodsScreen() {
  ...
  const hexes: FloodOverlayPoint[] = data?.hexes ?? [];
    ...
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.tabs}>
        {HORIZONS.map((h) => (
          ...
          >
            <Text style={[styles.tabLabel, horizon === h && styles.tabLabelActive]}>
              {t(`floods.tab.${h}`)}
            </Text>
          </Pressable>
        ))}
      ...
      <View style={styles.summary}>
        <Stat label={t("floods.stat.hexes")} value={hexes.length.toString()} />
        <Stat
          label={t("floods.stat.wet")}
          value={wet.toString()}
          tone={wet > 0 ? "hazard" : "good"}
        />
        <Stat label={t("floods.stat.mean")} value={`${mean}%`} />
      </View>

      {isPending && <Text style={styles.muted}>{t("floods.loading")}</Text>}
...
function HexRow({ hex }: { hex: FloodOverlayPoint }) {
